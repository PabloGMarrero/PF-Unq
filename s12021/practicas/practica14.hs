...
allPaths = foldM (\xs -> []) gn fb
  where gn [] = [[Straight]]
        gn r  = map (Straight :) r
        gb xs [] [] = [[Left], [Right]]
        gb xs [] rd = map (Right :) rd
        gb xs ri [] = map (Left  :) ri
        gb xs ri rd = map (Left  :) ri ++ map (Right :) rd
...
--preOrderGT = foldGT0 (\e rgts -> e : rgts)
--preOrderGT = foldGT1 (\e rgts -> e : concat rgts) (:) []
preOrderGT = foldGT z concat 
 where z x rgts = x : rgts

postOrderGT :: GTree a -> [a]
--postOrderGT (GNode e ts) = concat (map preOrderGT ts) ++ [e]
--postOrderGT = foldGT0 (\e rgts -> rgts ++ [e])
--postOrderGT = foldGT1 (\e rgts -> concat rgts ++ [e]) (:) []
postOrderGT = foldGT z concat 
 where z x rgts = rgts ++ [x]

--inOrderGT :: GTree a -> [a]

mirrorGT :: GTree a -> GTree a
--mirrorGT (GNode e ts) = GNode e (reverse (map mirrorGT ts))
--mirrorGT = foldGT0 (\e rgts -> GNode e (reverse rgts) )
--mirrorGT = foldGT1 (\e rgts -> GNode e (reverse rgts)) (:) []
mirrorGT = foldGT z reverse -- podria ser folGT	GNode reverse pero para demo se complia
 where z x rgts = GNode x rgts
 
countByGT :: (a -> Bool) -> GTree a -> Int
--countByGT f (GNode e ts) = if f e then 1 else 0 + sum (map (countByGT f) ts)
--countByGT = foldGT0 (\e rgts -> if f e then 1 else 0 + sum rgts)
--countByGT = foldGT1 (\e rgts -> if f e then 1 else 0 + sum rgts )) (+) 0
countByGT = foldGT (\e rgts -> if f e then 1 else 0 + rgts )) sum

partitionGT:: (a -> Bool) -> GTree a -> ([a], [a])
--partitionGT f (GNode e ts) = let (xs, ys) = aplanarPares (map (partitionGT f) ts) in if f e then (e:xs, ys) else (xs, e:ys)
partitionGT p = foldGT z g
 where z e (xs, ys) = if f e then (e:xs, ys) else (xs, e:ys)
       g xs     = aplanarPares xs
...
zipWithGT f (GNode e []) (GNode w []) = GNode (f e w) []
zipWithGT f (GNode e xs) (GNode w []) = GNode (f e w) []
zipWithGT f (GNode e (x:xs) (GNode w (y:ys) = GNode (f e w) [] -- no entiendo el caso recursivo

caminoMasLargoGT :: GTree a -> [a]
--caminoMasLargoGT (GNode e ts) = maxOrd e concat (map (caminoMasLargoGT) ts)
--caminoMasLargoGT = foldGT0 (\e rgts -> if f e then 1 else 0 + sum rgts)
--caminoMasLargoGT = foldGT1 (\e rgts -> if f e then 1 else 0 + sum rgts )) (+) 0
caminoMasLargoGT = foldGT (\e rgts -> if f e then 1 else 0 + rgts )) sum
...
foldFS0::(Name -> [b] -> b) -> (Name -> Content -> b) -> FileSystem -> b
foldFS0 f g (File n co) = g n co
foldFS0 f g (Folder n fs) = f n (map (foldFS0 f g) fs) 

foldFS1 :: (Name -> Content -> b) -> (Name -> c -> b) -> (b -> c -> c) -> c -> FileSystem -> b
foldFS1 f g h z (File n co) = f n co
foldFS1 f g h z (Folder n fs) = g n (foldr h z (map (foldFS1 f g h z) fs))

foldFS :: (Name-> Content -> b) -> (Name -> b -> b) -> ([b] -> b) -> FileSystem -> b
foldFS f g h (File n m ) = f n m
foldFS f g h (Folder n fs) =  g n (h (map (foldFS f g h) fs)) 

recFS0::(Name -> [FileSystem] -> [b] -> b) -> (Name -> Content -> b) -> FileSystem -> b
recFS0 f g (File n co) = g n co
recFS0 f g (Folder n fs) = f n fs (map (recFS0 f g) fs) 

recFS1 :: (Name -> Content -> b) -> (Name -> c -> b) -> (b -> c -> c) -> c -> FileSystem -> b
recFS1 f g h z (File n co) = f n co
recFS1 f g h z (Folder n fs) = g n (foldr h z (map (recFS1 f g h z) fs))

recFS :: (Name -> Content -> b) -> (Name -> [FileSystem] -> b -> b) -> ([b] -> b) -> FileSystem -> b
recFS f g h (File n m ) = f n m
recFS f g h (Folder n fs) =  g n fs (h (map (recFS f g h) fs)) 

-- b. Definir las siguientes funciones sin utilizar recursión explícita:

amountOfFiles :: FileSystem -> Int -- que describe la cantidad de archivos en el filesystem dado.
amountOfFiles = foldFS ff fo sum
 where ff n c   = 1
       fo n rfs = rfs

find :: Name -> FileSystem -> Maybe Content -- que describe el contenido del archivo con el nombre dado en el filesystem dado.
find n = foldFS ff fo aplanarMaybe
 where ff na c   = if n == na then Just c else Nothing
       fo na rfs = rfs
      
aplanarMaybe [m] = if isNothing m then Nothing else m
aplanarMaybe (m:ms) = aplanarMaybe ms

isNothing Nothing = True
isNothing _       = False

pathOf :: Name -> FileSystem -> Path -- que describe la ruta desde la raiz hasta el nombre dado en el filesystem dado. Precondición: el nombre existe en el filesystem.
pathOf n = recFS ff fo concat
 where ff na c   = if n == na then [na] else []
       fo na fs rfs = if existInFs na fs then na : rfs else rfs

existInFs:: Name -> [FileSystem] -> Bool
existInFs n []     = False
existInFs n (f:fs) = (not . null) (pathOf n f) || existInFs n fs


fs = Folder "Juegos" [Folder "Accion" [
                                        File "GOW" "", 
                                        File "Horizon Zero" "", 
                                        File "The Last of Us" ""
                                      ], 
                      ...
                      Folder "Deportivos" [
                                        File "PES 2020" "", 
                                        File "Fifa 2020" "es fifa 2020"
                                      ]
                    ]



mapContents :: (Content -> Content) -> FileSystem -> FileSystem --que describe el filesystem resultante de transformar todos los archivos en el filesystem dado aplicando la función dada.
mapContents f = foldFS g h id
 where g n cm  = File n (f cm)
       h n rfs = Folder n rfs


targetedMapContents :: [(Name, Content -> Content)] -> FileSystem -> FileSystem --, que describe el filesystem resultante de transformar el filesystem dado aplicando la función asociada a cada archivo en la lista dada
targetedMapContents f = foldFS g h id
 where g n cm  = File n (f n cm)
       h n rfs = Folder n rfs
